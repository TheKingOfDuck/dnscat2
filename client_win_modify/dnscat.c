...
"   port=<port>           The port to listen on (default: 5353).\n"
...
" ./dnscat --dns domain=skullseclabs.org,server=8.8.8.8,port=5353\n"
" ./dnscat --dns domain=skullseclabs.org,port=53535353\n"
" ./dnscat --dns domain=skullseclabs.org,port=5353,type=A,CNAME\n"
#if 0
" --tcp port=1234\n"
" --tcp port=1234,host=127.0.0.1\n"
#endif
"\n"
"By default, a --dns driver on port 5353 is enabled if a hostname is\n"
...
driver_dns_t *create_dns_driver_internal(select_group_t *group, char *domain, char *host, uint16_t port, char *type, char *server)
{
  if(!server && !domain)
  {
    ...
    printf("*\n");
    printf("*     dnscat --dns=server=1.2.3.4,port=5353\n");
    printf("*\n");
...
/*          session = session_create_ping(group, "ping");
  ...
  if(getenv("DNSCAT_DOMAIN")!=NULL) {
    ...
    tunnel_driver = create_dns_driver_internal(group, getenv("DNSCAT_DOMAIN"), "0.0.0.0", 5353, DEFAULT_TYPES, NULL);
    tunnel_driver_created = TRUE;
  }
  ...
  if(!tunnel_driver_created)
  {
    ...
    if(optind >= argc)
    {
      ...
      tunnel_driver = create_dns_driver_internal(group, NULL, "0.0.0.0", 5353, DEFAULT_TYPES, NULL);
    }
    else
    {
      tunnel_driver = create_dns_driver_internal(group, argv[optind], "0.0.0.0", 5353, DEFAULT_TYPES, NULL);
    }
  }
